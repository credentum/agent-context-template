...
def validate_implementation_phase(issue_number: int) -> bool:
    ...
    try:
        ...
        task_template_paths = list(Path("context/trace/task-templates").glob(f"*issue-{issue_number}*"))
...
def _is_code_file(file_path: str) -> bool:
    ...
    config_files = [
        ...
        'setup.py', 'requirements.txt', 'Dockerfile', 'docker-compose.yml'
    ]
...
def _verify_single_criterion(criterion: str) -> bool:
    ...
    if 'test' in criterion_lower and ('unit' in criterion_lower or 'comprehensive' in criterion_lower):
