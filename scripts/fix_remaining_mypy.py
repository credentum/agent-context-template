...
ALL_FIXES = {
    "tests/test_metadata_validation.py": [
        ...
        (
            ...
            'assert float(aggregation_metadata["min_value"]) <= '
            'float(aggregation_metadata["avg_value"])',
        ),
        (
            347,
            'assert aggregation_metadata["avg_value"] <= aggregation_metadata["max_value"]',
            'assert float(aggregation_metadata["avg_value"]) <= '
            'float(aggregation_metadata["max_value"])',
        ),
        (
            348,
    ...
    "tests/test_integration_ci_workflow.py": [
        ...
        (
            372,
            'actual_burned = burndown_data[0]["remaining_points"] - '
            'burndown_data[-1]["remaining_points"]',
            'actual_burned = int(burndown_data[0].get("remaining_points", 0)) - '
            'int(burndown_data[-1].get("remaining_points", 0))',
        ),
        (
            377,
        ...
        (
            403,
            'assert workflow_dispatch["inputs"]["sprint_id"].startswith("sprint-")',
            'assert str(workflow_dispatch.get("inputs", {}).get("sprint_id", ""))'
            '.startswith("sprint-")',
        ),
        (
            404,
            'assert workflow_dispatch["inputs"]["update_type"] in ["full", "delta"]',
            'assert workflow_dispatch.get("inputs", {}).get("update_type") in ["full", "delta"]',
        ),
        (
            405,
            'assert workflow_dispatch["inputs"]["validate_only"] is False',
    ...
    "tests/test_context_kv_methods.py": [
        ...
        (152, "result = kv.record_event(", "result = kv.record_event(  # type: ignore[call-arg]"),
    ],
