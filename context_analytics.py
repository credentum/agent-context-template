...
class ContextAnalytics(DuckDBAnalytics):
        ...
        try:
            ...
            if results:
                ...
                metrics = {
                    ...
                    'churn_rate': float(df['archived'].sum() / max(df['active_documents'].sum(), 1)),
                    'update_frequency': float(df['updated'].sum() / max(df['active_documents'].sum(), 1))
                }
                
                # Generate insights
                insights = []
                if metrics['churn_rate'] > 0.2:
                    insights.append(f"High document churn rate ({metrics['churn_rate']:.1%}) indicates frequent archiving")
        ...
        try:
            ...
            if results:
                ...
                for agent in results:
                    success_rate = agent['successes'] / max(agent['successes'] + agent['failures'], 1)
                    ...
                    if success_rate < 0.8:
                        insights.append(f"Agent {agent['agent_id']} has low success rate ({success_rate:.1%})")
                    
                    if agent['avg_duration'] and agent['avg_duration'] > 30:
                        insights.append(f"Agent {agent['agent_id']} has high average duration ({agent['avg_duration']:.1f}s)")
                
                # Overall metrics
                total_successes = sum(r['successes'] for r in results)
                total_failures = sum(r['failures'] for r in results)
                metrics['overall_success_rate'] = total_successes / max(total_successes + total_failures, 1)
                ...
                for agent_id, agent_data in metrics['agent_metrics'].items():
                    ...
                    if (now - last_active).days > 1:
                        insights.append(f"Agent {agent_id} has been inactive for {(now - last_active).days} days")
        ...
        try:
            ...
            for metric_name, metric_data in metrics['system_metrics'].items():
                if 'cpu' in metric_name and metric_data['max_value'] > 80:
                    insights.append(f"High CPU usage detected (max: {metric_data['max_value']:.1f}%)")
                ...
                if 'memory' in metric_name and metric_data['max_value'] > 90:
                    insights.append(f"High memory usage detected (max: {metric_data['max_value']:.1f}%)")
                    recommendations.append("Review memory usage patterns and optimize")
                
                if 'latency' in metric_name and metric_data['avg_value'] > 1000:
                    insights.append(f"High latency detected (avg: {metric_data['avg_value']:.0f}ms)")
            ...
            health_score -= min(metrics['warning_count'] / 50, 20)  # Max 20 point penalty for warnings
    ...
    def generate_executive_summary(self, days: int = 30) -> Dict[str, Any]:
        ...
        if reports['document_lifecycle']:
            summary['key_metrics']['active_documents'] = reports['document_lifecycle'].metrics.get('total_active', 0)
            summary['key_metrics']['document_churn'] = reports['document_lifecycle'].metrics.get('churn_rate', 0)
        
        if reports['agent_performance']:
            summary['key_metrics']['agent_success_rate'] = reports['agent_performance'].metrics.get('overall_success_rate', 0)
            summary['key_metrics']['total_agent_actions'] = reports['agent_performance'].metrics.get('total_actions', 0)
        
        if reports['system_health']:
            summary['key_metrics']['system_health_score'] = reports['system_health'].metrics.get('health_score', 0)
            summary['key_metrics']['error_count_24h'] = reports['system_health'].metrics.get('error_count', 0)
        
        # Compile top insights
        for report_name, report in reports.items():
            if report and report.insights:
                summary['top_insights'].extend([
                    f"[{report_name}] {insight}" 
...
@click.option('--report-type', type=click.Choice(['document', 'agent', 'health', 'all']), 
