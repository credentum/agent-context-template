...
class MockedBidirectionalWorkflowTest(unittest.TestCase):
    ...
    def _mock_subprocess_run(self, cmd, **kwargs):
        ...
        # Mock issue view
            ...
            if "--json" in cmd and "state" in cmd:
                ...
                return Mock(
                    returncode=0,
                    stdout=json.dumps({"labels": [{"name": "sprint-current"}, {"name": "phase:4.1"}]}),
                    stderr=""
                )
            else:
                return Mock(
                    returncode=0,
                    stdout=f"Issue #{issue_number} - Test Issue",
                    stderr=""
                )
    ...
    def _add_test_task_to_sprint(self):
        ...
        test_task = {
            ...
            "priority": "high",
            "assignee": "@claude-ai",
            "labels": ["sprint-current", "phase:4.1", "component:testing", "priority:high", "type:test"],
            "validation_criteria": [
    ...
    def test_baseline_verification(self, mock_subprocess):
        ...
        has_valid_structure = any(key in sprint_data for key in ["metadata", "phases", "goals", "title"])
        ...
        print(f"✓ Baseline verification passed. Found {len(existing_issue_numbers)} existing issues: {existing_issue_numbers}")
