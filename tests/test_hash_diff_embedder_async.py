...
def embedder(temp_dir, mock_config, mock_perf_config):
    ...
    embedder = AsyncHashDiffEmbedder(
        ...
        perf_config_path=str(perf_file),
        verbose=True
    )
    ...
    def test_init_with_default_paths(self):
        ...
        with patch.object(Path, 'exists', return_value=False):
            embedder = AsyncHashDiffEmbedder(
                ...
                perf_config_path="nonexistent_perf.yaml"
            )
...
class TestMainFunction:
    ...
    async def test_main_function_success(self, temp_dir):
        ...
        test_args = [
            ...
            "--perf-config", str(perf_file),
            "--verbose"
        ]
        
        with patch("sys.argv", test_args):
            with patch("src.storage.hash_diff_embedder_async.AsyncHashDiffEmbedder") as mock_embedder_class:
