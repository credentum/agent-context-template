...
class TestSprintIssueLinkerCoverage:
    ...
    def test_find_task_in_issues(self, issue_linker):
        ...
        existing_issues = [
            {"number": 1, "title": "Sprint 1 Tasks", "body": "Tasks:\n- Implement feature A\n- Write tests"},
    ...
    def test_create_issue_failure(self, issue_linker):
        ...
        with patch('subprocess.run') as mock_run:
            mock_run.side_effect = subprocess.CalledProcessError(1, 'gh', stderr="Validation failed")
            with patch('click.echo'):
    ...
    def test_cli_command_with_sprint_id(self, tmp_path, sprint_data):
        ...
        try:
            with patch('src.agents.sprint_issue_linker.SprintIssueLinker.create_issues_from_sprint', return_value=0):
    ...
    def test_cli_command_update_labels(self, tmp_path):
        ...
        try:
            with patch('src.agents.sprint_issue_linker.SprintIssueLinker.update_sprint_labels', return_value=0):
                with patch('src.agents.sprint_issue_linker.SprintIssueLinker._check_gh_cli'):
                    from click.testing import CliRunner
                    runner = CliRunner()
