...
class TestVectorDBInitializer:
    ...
    def test_load_config_non_dict_content(self):
        ...
        try:
            with patch('click.echo') as mock_echo:
                ...
                assert exc_info.value.code == 1
                mock_echo.assert_called_with(f"Error: {temp_path} must contain a dictionary", err=True)
        finally:
            import os
            os.unlink(temp_path)

    @patch('src.storage.vector_db_init.get_secure_connection_config')
    ...
    def test_connect_failure(self, mock_echo, mock_client_class, mock_get_config):
        ...
        assert initializer.client is None
        mock_echo.assert_called_with("✗ Failed to connect to Qdrant at localhost:6333: Connection failed", err=True)
