...
class SumScoresAPI:
    ...
    def search_single(self, query_vector: List[float], limit: int = 10,
    ...
    def search_multi(self, query_vectors: List[List[float]], limit: int = 10,
                    aggregation: str = 'sum', filter_conditions: Optional[Dict[str, Any]] = None) -> List[SearchResult]:
        ...
        for query_vector in query_vectors:
            results = self.search_single(query_vector, limit=limit*2, filter_conditions=filter_conditions)
    ...
    def search_contextual(self, query_vector: List[float], context_doc_ids: List[str],
                         limit: int = 10, context_weight: float = 0.3) -> List[SearchResult]:
        ...
        for doc_id in context_doc_ids:
            try:
                ...
                context_results = self.search_single([0.0]*1536, limit=1, filter_conditions=filter_cond)
                if context_results:
        ...
        if context_vectors:
            for result in base_results:
                ...
                for context in context_vectors:
                    ...
                    elif result.payload.get('sprint_number') == context.payload.get('sprint_number'):
...
def search(query: str, limit: int, doc_type: Optional[str], format: str):
    ...
    # Search
    results = api.search_single(query_vector, limit=limit, filter_conditions=filters if filters else None)
    ...
    if format == 'json':
        ...
        for i, r in enumerate(results, 1):
            ...
            click.echo(f"   Score: {r.final_score:.3f} (base: {r.score:.3f}, decay: {r.decay_factor:.2f}, boost: {r.boost_factor:.2f})")
