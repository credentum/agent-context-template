...
class GraphRAGIntegration:
    ...
    def _extract_reasoning_path(self, neighborhood: Dict[str, Any]) -> List[str]:
        ...
        if node_types:
            reasoning.append(f"Found {sum(node_types.values())} related documents: " + 
        ...
        if rel_types:
            reasoning.append("Relationships: " + 
                           ", ".join(f"{count} {t}" for t, count in rel_types.items()))
        
        # Analyze paths
        if neighborhood['paths']:
            avg_distance = sum(p['distance'] for p in neighborhood['paths']) / len(neighborhood['paths'])
            reasoning.append(f"Average connection distance: {avg_distance:.1f} hops")
        
        return reasoning
    
    def _generate_summary(self, query: str, vector_results: List[Dict[str, Any]], 
                         neighborhood: Dict[str, Any]) -> str:
        ...
        for i, result in enumerate(vector_results[:3], 1):
            context_parts.append(f"{i}. {result['title']} ({result['document_type']}) - Score: {result['score']:.3f}")
    ...
    def search(self, query: str, query_vector: List[float], 
              max_hops: int = 2, top_k: int = 5) -> GraphRAGResult:
        ...
        avg_combined_score = sum(combined_scores.values()) / len(combined_scores) if combined_scores else 0
        ...
        try:
            with self.neo4j_driver.session(database=self.database) as session:
                ...
                if impact['total_reachable'] > 0:
                    impact['central_score'] = impact['direct_connections'] / impact['total_reachable']
