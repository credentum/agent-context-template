...
class HashDiffEmbedder:
        ...
        if self.hash_cache_path.exists():
            try:
                with open(self.hash_cache_path, "r") as f:
                    ...
                    return {k: DocumentHash(**v) for k, v in cache_data.items()}
    ...
    def _save_hash_cache(self) -> None:
        ...
        cache_data = {k: asdict(v) for k, v in self.hash_cache.items()}
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    response = client.embeddings.create(
                        model=self.embedding_model, input=embedding_text
                    )
                    break
