#!/usr/bin/env python3
"""
hash_diff_embedder_async.py: Async version of the hash-diff embedder for better performance

This component:
1. Provides async/batch processing for OpenAI embeddings
2. Implements better rate limiting and concurrency control
3. Supports parallel document processing
"""
...
class AsyncHashDiffEmbedder:
    """Async embedder with improved performance"""
    ...
    async def _process_embedding_task(self, task: EmbeddingTask) -> Optional[str]:
        ...
        try:
            ...
            # Get embedding
            embedding = await self._embed_with_retry(embedding_text)
    ...
    async def embed_directory(self, directory: Path) -> Tuple[int, int]:
        ...
        for i in range(0, len(tasks), self.batch_size):
            ...
            results = await asyncio.gather(
                *[self._process_embedding_task(task) for task in batch],
...
async def main():
    ...
    parser.add_argument('path', type=Path, default=Path('context'), 
                       help='Directory to process')
    parser.add_argument('--config', default='.ctxrc.yaml',
                       help='Configuration file')
    parser.add_argument('--perf-config', default='performance.yaml',
    ...
    embedder = AsyncHashDiffEmbedder(
        ...
        perf_config_path=args.perf_config,
